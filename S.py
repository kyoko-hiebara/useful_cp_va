...
def main():
    ...
    # ここでは、Cartesian座標 -> fractional座標 -> [0,1]で折り返し -> Cartesian座標
    L_inv = np.linalg.inv(lattice)
    ...
    # ※原子座標は単位セル内に折り返したもの (coords_wrapped) を用いる
    atom_colors = {
        'H': 'white',
        'C': 'gray',
        'N': 'blue',
    ...
    ax.set_ylabel("Y (Å)")
    ax.set_zlabel("Z (Å)")
    ax.set_title("Water Accessible Surface and Atoms")
